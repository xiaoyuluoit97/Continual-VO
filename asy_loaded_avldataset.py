...
class CLPairDataset():
    ...
    #@profile(precision=2,stream=open('memorytest/load_chunk.log','w+'))
    ...
    def _remove_data_from_memory(self,num_of_chunk):
        self.rgb_d_pairs[num_of_chunk] = None
        self.targets[num_of_chunk] = None
    ...
    #@profile(precision=2, stream=open('memorytest/processdata.log', 'w+'))
    def _process_data(self, scence_n_chunk_k, i):
        ...
        input_pairs = torch.Tensor(
            np.concatenate([prev_rgb, cur_rgb, prev_depth, cur_depth], axis=2)
        )
...
def make_dataset_generator(filelist, folder_path,flag):
    ...
    if flag == "eval":
        ...
        for list in filelist:
            print("Now is eval experience"+list)
            print("Now is test experience" + list)
...
def avl_data_set(device):
    ...
    #for i in range(int(48)):
        #TRAIN_FILE_LIST.append("null")
