...
TRAIN = "naive"
RESUME_PATH = "log/naive"
#RESUME_PATH = "log/naive_60ep"
#TIMES="12"
#RESUME_FILE = "ewc_Exp66_resume11time.pth"
...
class CustomSavePlugin(PluginMetric):
    ...
    def __init__(self, log_folder=None,cur_exp=None):
        ...
        gt_forward = f"gt_forward_{self.cur_exp}.csv"
        gt_left = f"gt_left_{self.cur_exp}.csv"
        gt_right = f"gt_right_{self.cur_exp}.csv"



        self.gt_forward = open(
            os.path.join(self.log_folder, gt_forward), "w"
        )
        self.gt_left = open(
            os.path.join(self.log_folder, gt_left), "w"
        )
        self.gt_right = open(
            os.path.join(self.log_folder, gt_right), "w"
        )
    ...
    def compute_abs_diff(
        ...
        for i in range(len(action)):
            if action[i] == 1:
                ...
                self.perexp_absdelta(eval_exp, action[i].item(),gt_deltax[i].item(),gt_deltayaw[i].item(),gt_deltaz[i].item())
            elif action[i] == 2:
                ...
                self.overall_pred_metrics(eval_exp, action[i].item(),
                                          pred_deltax[i].item(),pred_deltayaw[i].item(), pred_deltaz[i].item())
                self.perexp_absdelta(eval_exp, action[i].item(), gt_deltax[i].item(), gt_deltayaw[i].item(),
                                     gt_deltaz[i].item())
            elif action[i] == 3:
...
def main():
    ...
    #pth_files.sort(key=lambda x: int(x.split('_')[1][7:]))
    pth_files.sort(key=lambda x: int(x.split('_')[0][8:]))
    print(pth_files)
    if ESUME_TRAINR:
        for pth_file in pth_files[initexperience:]:
            ...
            if ESUME_TRAINR:
                # 使用正则表达式提取信息
                match = re.match(r".*60epExp(\d+)_", pth_file)
                if match:
