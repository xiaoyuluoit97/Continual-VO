...
import os
import numpy as np
folder_path = 'log/without_image/csvfile'  # 文件夹路径，请替换成你的文件夹路径
output_file_path = 'log/only_act'
...
for file_name in sorted_files:
    ...
    range_upper_x.append(np.mean(data.iloc[:,3]) + 2 * np.sqrt(np.var(data.iloc[:,3])))
    range_lower_x.append(np.mean(data.iloc[:, 3]) - 2 * np.sqrt(np.var(data.iloc[:, 3])))

    variance_yaw.append(np.var(data.iloc[:,4]))
    mean_yaw.append(np.mean(data.iloc[:,4]))
    range_upper_yaw.append(np.mean(data.iloc[:,4]) + 2 * np.sqrt(np.var(data.iloc[:,4])))
    range_lower_yaw.append(np.mean(data.iloc[:,4]) - 2 * np.sqrt(np.var(data.iloc[:,4])))

    variance_z.append(np.var(data.iloc[:,5]))
    mean_z.append(np.mean(data.iloc[:,5]))
    range_upper_z.append(np.mean(data.iloc[:,5]) + 2 * np.sqrt(np.var(data.iloc[:,5])))
    range_lower_z.append(np.mean(data.iloc[:,5]) - 2 * np.sqrt(np.var(data.iloc[:,5])))
