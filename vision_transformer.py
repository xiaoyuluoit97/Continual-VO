...
class VisionTransformer(nn.Module):
    ...
    def _preprocess_rgbd(self, observation_pairs):
        ...
        rgb1 = rgb_observations[:, : 3, :]
        rgb2 = rgb_observations[:, 3:, :]

        depth_observations = observation_pairs[:, :, :, -3:-1]
        # permute tensor to dimension [BATCH x CHANNEL x HEIGHT X WIDTH]
        depth_observations = depth_observations.permute(0, 3, 1, 2)
        depth1 = depth_observations[:, : 1, :]
        depth2 = depth_observations[:, 1:, :]
    ...
    def init_weights(self, mode: Literal['jax', 'jax_nlhb', 'moco', ''] = '') -> None:
        ...
        if self.cls_token is not None:
            ...
            nn.init.xavier_normal_(self.embed_liner.weight)
    ...
    def _pos_embed(self, x: torch.Tensor,action:torch.Tensor) -> torch.Tensor:
