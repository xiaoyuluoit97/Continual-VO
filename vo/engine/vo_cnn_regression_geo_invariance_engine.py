...
class VOCNNRegressionGeometricInvarianceEngine(VOCNNBaseEngine):
    ...
    def _process_one_batch(
        ...
        for act in self._act_list:
            #in here we do a filter. only keep action 1,2,3. action 4 is the back which we do not need it.
            if act == -1: ## all action use one model! our case adesso
                act_mask = actions.eq(4)
                act_mask = ~act_mask
                act_mask_flat = act_mask.view(-1)
                act_idxes = torch.arange(actions.size(0))[act_mask_flat]
            else:
            ...
            for i, d_type in enumerate(self.delta_types):
                ...
                if len(cur_geo_invariance_types) == 0:
                    ...
                    #to here we want all action because we use all 1,2,3 action to got the sample error pred loss.
                    #not invariance yet
                    loss += d_loss
        ...
        if "inverse_joint_train" in cur_geo_invariance_types:
            ...
            #worked at here. filter the left & right then we do it.
