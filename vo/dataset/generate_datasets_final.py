...
def save_data_to_disk(group, all_data):
    ...
    group.create_dataset("whole_h5_indexs", data=whole_h5_indexs, **H5PY_COMPRESS_KWARGS)
    group.create_dataset("num_of_episodes", data=num_of_episodes, **H5PY_COMPRESS_KWARGS)
    group.create_dataset("num_of_frame_in_episodes", data=num_of_frame_in_episodes, **H5PY_COMPRESS_KWARGS)
...
def generate_one_dataset(
    ...
    ## whole data point number divide whole scenes number
    # ## how much steps in each scene
    ...
    cur_scene_steps = 0
    cnt = 0
    episode_cnt_scence = 0
    whole_episode_counter = 1
    cur_episode_steps = 0
    create_new_episode = 1
    ...
    with h5py.File(
        ...
        while whole_episode_counter <= (num_of_scences+1) * num_of_episodes_per_scence:
            ...
            if create_new_chunk == 1:
                ...
                # save previsou chunk
                # if episode_cnt != 0:
                if whole_episode_counter != 1:
                    group = f.create_group(f"scene_{cur_scene}_chunk_{chunk_cnt}")
                    chunk_cnt += 1

                    print("Convert data into np.array ...")
                    tmp_start = time.time()
                    whole_h5_indexs = np.array(whole_h5_indexs, dtype=np.uint16)
                    new_episodes = np.array(new_episodes, dtype=np.uint8)
                    num_of_episodes = np.array(num_of_episodes, dtype=np.uint8)
                    num_of_frame_in_episodes = np.array(num_of_frame_in_episodes, dtype=np.uint8)
            ...
            #Scence create here!!!!!!!!
            if env is None or episode_cnt_scence > num_of_episodes_per_scence:
                if env is not None:
                    env.close()
            ...
            while not env.episode_over and episode_cnt_scence <= num_of_episodes_per_scence:
                ...
                if best_action in valid_act and np.random.binomial(1, rnd_p) == 1:
                    ...
                    if cnt % LOG_INTERVAL == 0:
                        ...
                        tmp_time_per_ep = tmp_time_interval / episode_cnt_scence
                        # tmp_remain_time = tmp_time_interval * (N - cnt) / cnt
                        print(
                            f"[{cnt}] "
                            f"ep: {episode_cnt_scence}, {tmp_time_per_ep:.2f}s / episode; "
                            # f"remain: {tmp_remain_time:.2f}s\n"
                        )

                    #chunk_data_cnt += 1
                    # if whole_episode_counter % Episode_per_chunk == 0:
                    #     create_new_chunk = 1
                    if episode_cnt_scence % Episode_per_chunk == 0 :
                        create_new_chunk = 1
                    ...
                    num_of_episodes.append(episode_cnt_scence)
                    whole_h5_indexs.append(whole_h5_index)
                    num_of_frame_in_episodes.append(cur_episode_steps)
    ...
    return episode_cnt_scence, action_cnt, {"collision": collision_cnt}
