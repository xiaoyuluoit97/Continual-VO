...
def save_data_to_disk(group, all_data):
    ...
    group.create_dataset("split_flags", data=split_flags, **H5PY_COMPRESS_KWARGS)
        ...
        while cnt <= N:
            if create_new_chunk == 1:
                ...
                if cnt != 0:
                    ...
                    tmp_start = time.time()
                    split_flags = np.array(split_flags,dtype=np.uint8)
                    new_episodes = np.array(new_episodes, dtype=np.uint8)
                    whole_h5_indexs = np.array(whole_h5_indexs,dtype=np.uint16)
                    num_of_episodes = np.array(num_of_episodes, dtype=np.uint16)
                    num_of_frame_in_episodes = np.array(num_of_frame_in_episodes, dtype=np.uint16)
                    ...
                    if num_of_episodes is not None :
                        print("num of episodes not null")
                        print(type(num_of_episodes))
                        print(len(num_of_episodes))

                    if num_of_frame_in_episodes is not None :
                        print("num_of_frame_in_episodes not null")
                        print(type(num_of_frame_in_episodes))
                        print(len(num_of_frame_in_episodes))
                    if num_of_episodes is not None :
                        print("episode_start_positions not null")
                        print(type(episode_start_positions))
                        print(len(episode_start_positions))
            ...
            if best_action in valid_act and np.random.binomial(1, rnd_p) == 1:
                ...
                whole_h5_index += 1
                num_of_episodes.append(episode_cnt)
                num_of_frame_in_episodes.append(cur_episodes_frame)
...
def main():
    ...
    print(args.train_scene_dir)
    print(args.val_scene_dir)
