    ...
    def layer_init(self):
        for layer in self.modules():
            if isinstance(layer, (nn.Conv2d, nn.Linear)):
                print("im init with relu,im init with relu,im init with relu,im init with relu,im init with relu,im init with relu,im init with relu,im init with relu")
...
class ResNetEncoderWithActChannle(nn.Module):
    def __init__(
        self,
        *,
        observation_space,
        ...
        if "top_down_view" in observation_space:
            ...
            self._n_input_top_down_view = 0
        if "act_emb" in observation_space:
            spatial_size_w, spatial_size_h = observation_size
            self._act_channle = act_channle
    ...
    def forward(self, observation_pairs):
        ...
        if self._act_channle > 0:
            acts_channle = observation_pairs["act_emb"]
            acts_channle = acts_channle.permute(0, 3, 1, 2)
            cnn_input.append(acts_channle)
...
class VisualOdometryCNNWithChannelBase(nn.Module):
    def __init__(
        self,
        *,
        observation_space,
        ...
        self.visual_encoder = ResNetEncoderWithActChannle(
            observation_space=observation_space,
            observation_size=observation_size,
            baseplanes=resnet_baseplanes,
            ngroups=resnet_baseplanes // 2,
